...
def new_route(start_point=None, end_point=None):
    ...
    if shortest_route is not None:
        ...
        print(
            f"The shortest metro toute from {start_point} to {end_point} is:\n{shortest_route_string}"
        )
    else:
        print(
            f"Unfortunately, there is currently no path between {start_point} and {end_point} due to maintenance."
        )
...
def get_route(start_point, end_point):
    ...
    for start_station in start_stations:
        for end_station in end_stations:
            metro_system = (
                get_active_stations() if stations_under_construction else vc_metro
            )
